...
class cv_data():
    ...
    def __sample__(self, num = 500, thres = 80, mpr = 30, balance = True, window_size = 600):
        ...
        while num_false < num or num_true < num :
            ...
            ''' section to predict '''
            sec_pd = df [ (df['Local_Y'] <  x + 400) &
                          (df['Local_Y'] >=  x ) &
                          (df['Global_Time'] >= t + 20000 ) &
                          (df['Global_Time'] < 	t + 30000 ) &
                          (df['Lane_ID'] < 6 )            ]
            res_pd = get_elements(sec_pd, [t+20000, t+30000], [x, x + 200])
            k_pd = res_pd["density"]
            status = False if k_pd < thres else True

            
            ''' current section '''
            sec_cr = cv [ (cv['Local_Y'] <  x + 400) &
                          (cv['Local_Y'] >=  x) &
                          (cv['Global_Time'] >= t - window_size * 100 ) &
                          (cv['Global_Time'] < 	t  ) &
                          (cv['Lane_ID'] < 6 )            ]
            res_cr = get_elements(sec_cr, [t - window_size * 100, t], [x, x + 200], plot = 0)

            ''' downstream section '''
            sec_ds = cv [ (cv['Local_Y'] <  x + 800) &
                          (cv['Local_Y'] >=  x + 400) &  
                          (cv['Global_Time'] >= t - window_size * 100) &
                          (cv['Global_Time'] < 	t  ) &
                          (cv['Lane_ID'] < 6 )            ]
            res_ds = get_elements(sec_ds, [t - window_size * 100, t], [x + 400, x + 800], plot = 0)
            ...
            if len(res_cr["vels"]) != window_size or len(res_ds["vels"]) != window_size or len(res_cr["stds"]) != window_size or len(res_ds["stds"]) != window_size:
                print ( len(res_cr["vels"]), len(res_ds["vels"]), len(res_cr["stds"]), len(res_ds["stds"]))
                continue
        ...
        samples_sdc = pd.DataFrame(zip(np.array(vel_cr), np.array(sdc_cr), np.array(std_cr), np.array(vel_ds), np.array(sdc_ds), np.array(std_ds), label, zones, windows), columns = ["current speed series", "current sdc series", "current std series", "downstream speed series", "dowmstream sdc series", "dowmstream std series", "status", "zone", "window"])
    ...
    def __cross_validation__(self, samples = None, K = 5):
        ...
        ''' get features '''
        features = np.zeros((N, 6, 600))
        for i in range(N):
            for j in range(len(samples[i][:6])):
...
def get_elements(df, window, zone, plot = False):
    ...
    return {"speed": speed, "density": density, "flow": flow, "covs": np.asarray(covs), "frames": ts, "ssd": ssd, "vels": np.asarray(vels), "cors":np.asarray(cors), "stds":np.asarray(stds)}    
...
if __name__ == "__main__":  
    ...
    # us101 = cv_data()
    # us101.__import_samples__("./data/sdc_samples_30_09_01_2021_16_31_48")
    ...
    def __task__():
        ...
        print (len(frames))
        ax2.plot(frames, _smooth_(stds), linewidth = 0.3, color = "red", zorder = 1)
