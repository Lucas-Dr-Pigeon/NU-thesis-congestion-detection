    ...
    def __sample__(self, num = 500, thres = 80, mpr = 30, balance = True, window_size = 600):
        ...
        while num_false < num or num_true < num :
            ...
            sec_pd = df [ (df['Local_Y'] <  x + 200) &
                          ...
                          (df['Lane_ID'] < 6 )            ]
            res_pd = get_elements(sec_pd, t+0, t+30000)
            k_pd = res_pd["density"]
            ...
            sec_cr = cv [ (cv['Local_Y'] <  x + 200) &
                          ...
                          (cv['Global_Time'] < 	t  ) &
                          (cv['Lane_ID'] < 6 )            ]
            res_cr = get_elements(sec_cr, t - window_size * 100, t )
            ...
            sec_ds = cv [ (cv['Local_Y'] <  x + 400) &
                          ...
                          (cv['Global_Time'] < 	t  ) &
                          (cv['Lane_ID'] < 6 )            ]
            res_ds = get_elements(sec_ds, t - window_size * 100, t)
            ...
            if len(res_cr["vels"]) != window_size or len(res_ds["vels"]) != window_size or len(res_cr["covs"]) != window_size or len(res_ds["covs"]) != window_size:
                print ( len(res_cr["vels"]), len(res_ds["vels"]), len(res_cr["covs"]), len(res_ds["covs"]))
                continue
...
def get_elements(df, ini_t, end_t):
    ...
    diff = np.setdiff1d(np.arange(ini_t, end_t, 100), np.asarray(list(gt.groups.keys())))
    ...
    for t in np.arange(ini_t, end_t, 100):
        x = df[df["Global_Time"] == t]["Local_Y"].values
        v = df[df["Global_Time"] == t]["v_Vel"].values
    ...
    ax1.plot(ts, np.asarray(vels), color = "green", linewidth = 1)
    ax1.set_ylim(0, 80)
    ax2 = ax1.twinx()
    ax2.plot(ts, _smooth_(cors), color = color, linewidth = 1)
...
if __name__ == "__main__":  
    ...
    # us101 = cv_data()
    # us101.__import_samples__("./data/sdc_samples_30_31_12_2020_23_37_55")
    ...
    # a = np.array([1, 2, 3])
    # b = np.array([200, 200.1, 200.45])
    # print (np.cov(a, b / 100))
    ...
    def __task__():
        T = [tuple([t *30000 + 1118847100000, ( t + 1 ) *30000 + 1118847100000]) for t in np.arange(0,20)  ]
        vs = []
        ks = []
        covs = []
        cors = []
        ...
        for window in T:
            ...
            res = get_elements(sec, window[0], window[1])
            vs.append(res["speed"])
