    ...
    def __sample__(self, num = 500, thres = 80, mpr = 30, balance = True, window_size = 600):
        ...
        while num_false < num or num_true < num :
            ...
            sec_pd = df [ (df['Local_Y'] <  x + 200) &
                          ...
                          (df['Lane_ID'] < 6 )            ]
            res_pd = get_elements(sec_pd, [t+0, t+30000], [x, x + 200])
            k_pd = res_pd["density"]
            ...
            sec_ds = cv [ (cv['Local_Y'] <  x + 400) &
                          ...
                          (cv['Global_Time'] < 	t  ) &
                          (cv['Lane_ID'] < 6 )            ]
            res_ds = get_elements(sec_ds, [t - window_size * 100, t], [x + 200, x + 400], plot = 0)
            ...
            sdc_cr = np.append(sdc_cr, [res_cr["covs"]], axis = 0)
            vel_ds = np.append(vel_ds, [res_ds["vels"]], axis = 0)
            sdc_ds = np.append(sdc_ds, [res_ds["covs"]], axis = 0)
...
if __name__ == "__main__":  
    ...
    us101 = cv_data()
    us101.__import_samples__("./data/sdc_samples_30_01_01_2021_20_50_00")
    ...
    def __task__():
        ...
        for window in T:
            print (window)
            sec = us101_data[ (us101_data['Local_Y'] < zone[1]) &
                      (us101_data['Local_Y'] >= zone[0]) &
                      (us101_data['Global_Time'] >= window[0] ) &
                      (us101_data['Global_Time'] < 	window[1] ) &
                      (us101_data['Lane_ID'] < 6 )            ]
            
            res = get_elements(sec, window, zone)
            vs.append(res["speed"])
        ...
        print (len(frames))
        ax2.plot(frames, _smooth_(covs), linewidth = 0.3, color = "red", zorder = 1)
