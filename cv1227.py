...
class cv_data():
    ...
    def __sample__(self, num = 500, thres = 80, mpr = 50, balance = True):
        ...
        while num_false < num or num_true < num :
            ...
            windows.append([ t + 10000, t + 20000 ] )
        ...
        ''' dump samples to a df '''
        samples = pd.DataFrame(zip(v_cr, ssd_cr, v_ds, ssd_ds, label), columns = ["current speed", "current ssd", "downstream speed", "dowmstream ssd", "status"])
        
        ''' save samples '''
        save_data(samples, "./data/ssd_samples_" + str(mpr) + "_" + str(dt_string))
    ...
    def __cross_validation__(self, samples = None, K = 5, mode = "NN"):
        ...
        samples[:,1] /= 15
        samples[:,3] /= 15
        samples[:,0] = samples[:,0] / 70
        samples[:,2] = samples[:,2] / 70
        ...
        for _k in range(K - 1):
            ...
            if mode == "NN":
                ...
                rfpreds = rf.predict(np.append(testset.x, validset.x, axis = 0))
                rfcm = confusion_matrix(rfpreds, np.append(testset.y, validset.y, axis = 0 ))
                ...
                machine = svm.SVC()
                machine.fit(trainset.x, trainset.y)
                svmpreds = machine.predict(np.append(testset.x, validset.x, axis = 0))
                svmcm = confusion_matrix(svmpreds, np.append(testset.y, validset.y, axis = 0))
        ...
        print ("Final Accuracy: " + str(np.mean(test_accuracies)))
        return test_accuracies, test_cms
...
def get_elements(df):
    ...
    return {"speed": speed, "density": density, "flow": flow, "covs": covs, "frames": ts, "ssd": ssd}    
        
def _smooth_(y, box_pts=20):
...
if __name__ == "__main__":
    ...
    us101 = cv_data()
    us101.__import_samples__("./data/ssd_samples_50_17_02_2021_23_45_12")
    ssd_result = us101.__cross_validation__(mode = "RF")
    ...
    def __task__():
        ...
        for window in T:
            print (window)
            sec = us101_data[ (us101_data['Local_Y'] <  900) &
                      (us101_data['Local_Y'] >= 700) &
